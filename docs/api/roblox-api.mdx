...
`.new()` functions (like `Vector3.new()` or `CFrame.new()`) should instead be called with the `new` operator. `new T(...)` will always compile to `T.new(...)`.
...
```ts
// note: Part could be inferred here if not provided
